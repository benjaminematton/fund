...
- `mcp__alpaca__place_*` — one call per open ticket, `client_order_id` set to the ticket id. Exit legs are FLAT parameters (`stop_loss_stop_price`, `stop_loss_limit_price`, `take_profit_limit_price`), never a nested object; the gate denies anything else and the broker ignores it.
