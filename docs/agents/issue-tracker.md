...
- **Decision child**: filed with `needs-decision` and a region, the same as any other child. It names a choice for a human, not a task to pick up, and the standup dispatches it as a *decide* lane whose deliverable is a decision package rather than a diff. A decision child that already enumerates two or more named options is not dispatched at all — it goes straight onto the standup's decision list, because a seat sent to restate it would only re-derive what the filer already wrote.
