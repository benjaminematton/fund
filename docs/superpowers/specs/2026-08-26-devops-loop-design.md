...
silent. `issue_coverage` is the backstop, but it only fires when `dev-status` runs, and
`dev-status` runs when standup does. **Filer silence is covered by the standup cadence, not
by machinery.** Worst case is a one-standup delay rather than eight hours of
nobody-owns-it; if standup lapses, both producers go silent together. Acceptable, and
...
2. **devcheck checks are pure functions over `Snapshot`** — the sweep check takes tracker
   rows through `Snapshot`, and `dev_status.py` stays the only place doing I/O. Existing
   pattern; `tracked_checks` is the precedent.
...
4. A condition suppressed in `health.md` produces nothing.
...
8. `ops/pull-backups.sh` invokes the filer only after a successful rsync, and a filer
   failure is reported without failing the pull.
