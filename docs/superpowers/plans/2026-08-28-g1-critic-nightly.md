...
**Bullet 4 caveat, corrected (see #163):** "the repo has no replay harness" is false. `agents/replay.py` ships both `load_recording()` and `replay_turn(decisions, *, pre_hooks, executor, post_hooks)`, exercised against real gate hooks, a real executor and a real DB by `tests/test_replay.py`, with twenty recordings tracked in `tests/recordings/`. What's missing is only the `make replay REC=` CLI wrapper — `Makefile:52-55` still exits 2 with "not implemented yet" — which #163 tracks as an open, unbuilt-CLI decision, not a missing capability.

`replay_turn` itself is tool-agnostic: it drives whatever `executor: Callable[[str, dict], object]` it is handed, so the hook/executor/DB wiring does not care whether a decision names an Alpaca order or a G1 verdict. The narrower gap for a *G1* replay is that `make_executor` (`tests/conftest.py:23`), the executor fixture the existing replay tests use, has a closed if/elif dispatch table covering only `mcp__alpaca__place_*`, `list_open_tickets`, `get_stage_brief`, `submit_signal` and `submit_decision` — it raises `ValueError` for anything else, including the Critic's two G1 tools, `get_spec_brief` and `submit_spec_critique`. Both already have handlers in `agents/tools/fund_server.py` (`handle_get_spec_brief`, `handle_submit_spec_critique`) with the same `(conn, seat=..., args=..., run_date=...)` shape as the branches already wired, so extending the fixture is two more `elif` branches following the existing pattern, not new machinery. **A G1 replay is therefore reachable, not blocked by `agents/replay.py`'s executor contract — it is unbuilt follow-up work, not attempted in this lane.** This plan's own testable content today stays prompt invariance, which is what the two tests above assert.
