...
- **Failure, three cases** (`improvement.md` §2.1, §6): (i) job crash → no row for any seat, last good rows stand, the brief carries them with their `as_of_date`, one alert; (ii) a non-finite load-bearing value for one seat → that seat skipped and named in one alert, every other seat written; (iii) `weights` empty → the section is named in `unavailable` and the PM proceeds with equal weights.
