...
Two qualifications sit on three rows, neither of them expressible in the columns. The G1 pair (`get_spec_brief`, `submit_spec_critique`) is enumerated here because the fund server serves it, but its schemas stay in `specs/strategy-contracts.md` §3.4 and that file remains their authority — restating them here would create the second source of truth this table exists to prevent. And `submit_critique` is specified below but **not served**: the Critic seat runs G1 only, and the trade-pipeline critique is Phase 3 (`specs/design.md`; the Decision stage runs as a single turn on the orchestrator's own `no_critic_seat` rows until then). `status` is exactly `served` or begins `not served`; anything else fails the test rather than being interpreted.

`submit_strategy_spec` was the third qualification until #198, and the record of why it stopped being one belongs here rather than only in a commit message. It shipped in #171 with a handler, a §4 row and **no `@tool` and no cap** (🔏 ruling 2026-08-29, G-2(iii)): nothing in `orchestrator/` or `scripts/` assigned a turn that registered a spec, so a cap would have widened a *trading* seat's write surface with a tool its charter never mentioned and nothing ever called — zero function at a cost in surface honesty. #198 removed that condition rather than relaxing the rule. It staffs the Quant Researcher that `specs/design.md`'s seat table already names, **as a registration seat only**: one cap, no brief, and no read tool of any kind. That is deliberately narrower than the design-table row, which lists a `stock-data` toolset — `agents/config/quant.yaml` records the reason, that a standing surface wider than any turn the seat runs is surface nobody is using. `scripts/register_spec.py`, run by hand through `make register-spec`, is what assigns the turn. The seat is **offline only**: it has no entry in `scripts/run_day.py`'s `SEATS`, so no trading-day turn carries this cap — the property the earlier ruling was protecting — and `tests/test_submit_strategy_spec.py` asserts that absence executably rather than in prose. There is still no timer behind it, because `specs/strategy.md`'s *PM sponsors → SPEC* transition has no implementing mechanism; the human invocation stands in for the missing sponsorship gate. #197 landed the `strategies` row: `state/specs.py:insert_strategy_spec` now INSERTs both the `strategy_specs` row and the `strategies` lifecycle row in one commit, so the handler conforms to `strategy-contracts.md` §3.1 in full.
