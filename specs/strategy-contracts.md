...
> **Unification status:** the shipped `fundbt/registry.py` maintains its own minimal DDL for `trial_registry` + `holdout_evaluations` (standalone, `:memory:` default, no FKs) so the starter kit runs without the fund DB. The schema below is canonical; migrating the registry to write to the fund DB (FKs intact, single source of truth) is the Phase-5 acceptance item "Trial registry unified." `sleeves` and `shadow_fills` have **no implementing code yet** — they are Phase-5 integration work. `strategy_specs`, `strategies` and `strategy_critiques` are live in `state/schema.sql`. `strategy_specs` and `strategies` share one write path, `state/specs.py:insert_strategy_spec`, which INSERTs the spec and its lifecycle row in state `SPEC` in a single transaction (§3.1); `strategy_critiques`' write path is `submit_spec_critique` (§3.4). Three paths READ `strategy_critiques`: `state/specs.py:specs_awaiting_critique`'s queue predicate, `submit_spec_critique`'s write-once guard, and `scripts/critic_g1.py`'s post-turn re-read. (A note here used to say nothing yet read it, deferring G1 enforcement to a separate change; it entered in the same commit as the selector's `LEFT JOIN strategy_critiques` and was never true. It predates #197 — `scripts/critic_g1.py:6` still cites it.) Nothing yet TRANSITIONS a `strategies` row, however: §4's edges have no implementing code and `state/transition.py`'s `EDGES` carries no entry for the table, so a row written at registration stays in `SPEC`.
>
> **Correction, closed by #197.** The note that stood here claimed `strategies.state == 'SPEC'` and "has no `strategy_critiques` row" were *equivalent* conditions, so that the second could stand in for the first until the table landed. **That claim was wrong.** §4's transition table has no G1 edge, so writing a verdict moves nothing out of `SPEC`: a `state = 'SPEC'` selector on its own re-selects a spec that has already been critiqued, and `submit_spec_critique` is write-once (§3.4), so the second verdict is refused, the turn fails, and the queue head blocks every spec behind it. The conditions are therefore conjunctive, not interchangeable — **a spec awaits review when its `strategies` row is in state `SPEC` AND it has no `strategy_critiques` row.** That is the one selector, in `state/specs.py:specs_awaiting_critique`. The critique conjunct is what retires reviewed specs and is required until §4 grows a G1 edge; the state conjunct is an INNER JOIN, so a spec with no lifecycle row is excluded rather than assumed pending.
...
`expected_spec_id` is a **handler parameter bound by the caller, never by the seat** — the same mechanism `handle_submit_reflection` uses for `expected_decision_id` (contracts.md §2), threaded caller → `build_seat_options` → `build_fund_server` → handler. The nightly G1 job binds it to the queue head it served the turn. The handler refuses when it is `None` — an unbound turn must not write a verdict blind — and refuses when `args["spec_id"]` names any other spec. The seat's `spec_id` argument therefore stays in the schema and becomes a **checked assertion**: the seat still says which spec it is judging, and the handler now knows whether that is the spec it was shown.

It is `None`-defaulted and refused rather than REQUIRED like `charter_version`/`model_id`, and the difference is not style: those are available at every call site, while this exists only on the Critic's path — `build_fund_server` is built per seat and has nothing to bind for a seat that will never call this tool. The refusal is what makes the optional parameter safe.

This is deliberately narrower than reflect's binding, which *replaces* the id outright (`decision_id = expected_decision_id`) because `submit_reflection`'s schema carries no `decision_id` at all. Here `spec_id` is a **required** schema field, and a required field the handler ignores is a trap for the next reader. Refusing a mismatch preserves the field's meaning and gives the failure a name at the call site.

Why it must exist: `submit_spec_critique` is write-once. A turn shown spec A that writes a verdict for spec B leaves **B permanently unreviewable** through any shipped path, while A keeps blocking the queue head. Post-hoc detection — the nightly job's `has_verdict` re-read — reports A's missing verdict, which is the symptom, and can neither see nor undo B's consumption. Detection after an irreversible write is not a mitigation, so this is a binding rather than a check.

**`get_spec_brief` is unaffected and stays argument-less.** Binding the WRITE does not unblock the QUEUE: the selector still returns the oldest unreviewed spec, so a spec the seat cannot critique still blocks every spec behind it. That is a separate defect needing a separate decision (issue #182, second half) and it is the one that would require changing this section's read half.

The read half is `get_spec_brief` (Critic only, no arguments): the oldest spec whose `strategies` row is in state `SPEC` **and** that carries no `strategy_critiques` row — both conjuncts, per §2's correction, never either alone — one per turn, JSON columns decoded, plus the seat's own journal. The journal degrades like `get_stage_brief`'s sections (contracts §4): unbuildable means empty plus a name in `unavailable`. **The spec queue does not degrade.** An unreadable queue returns an error rather than `[]`, because an empty list is indistinguishable from "nothing pending" and would end the turn with an unreviewed spec behind a clean-looking trace. A registered spec with no `strategies` row is such an error and not a silent exclusion: `state/specs.py` raises `OrphanedSpecs`, naming every affected spec and the repair, rather than letting the inner join drop it.
