...
| Proposer | Monthly: reads the scoreboard and the record for one bound target seat, submits ≤1 pre-registered proposal (threshold, seat config, watchlist, tool contract; charter/desk only after `improvement.md` §3.5) — a human merges or refuses (`specs/improvement.md` §3) | fast | none | no |
...
| Monthly | Propose | the `proposer` seat submits ≤1 pre-registered proposal per seat per open window; code projects it as a PR or `#ceo-office` post; the human merges or refuses; at horizon, code grades the prediction (`specs/improvement.md` §3) |
