...
4. **The improvement loop** — the night and the month, not the trading day: nightly `resolve → reflect → score → apply` (no LLM — weights, kill rules, desk narrowing), weekly lessons distillation, monthly proposals a human commits. Specified in `specs/improvement.md`; the trading day above reads its outputs (`weights`, lessons) as data.
...
| Ops | Standup, EOD digest, the daily scorecard (`scripts/score_day.py` — turns ranked for a human to read) and the calibration scoreboard's weekly projection (computed by code, never by a seat), invalidation watch | fast | `account` | no |
...
| Distill | Weekly, per seat: regenerates that seat's ≤40-line lessons file from its journal, resolutions and scoreboard slice — never its own record (`specs/improvement.md` §2.4) | fast | none | no |
| Proposer | Monthly: reads the scoreboard and the record for one bound target seat, submits ≤1 pre-registered proposal (threshold, seat config, watchlist, tool description; charter/desk only after `improvement.md` §3.5) — code opens the PR, a human merges or refuses (`specs/improvement.md` §3) | fast | none | no |
| ↳ Both are fenced like the exec seat, not like a dev seat: `setting_sources=[]`, explicit `tools` allow-array of their two fund tools, no Alpaca toolset, no Slack write. Their targets are bound server-side for the turn — the seat cannot choose whose record it reads or what it proposes against. Neither runs in the daily cycle. |||||
...
| 08:45 | Pre-gate | Gate computes **allowed actions** `{buy: max_qty, sell: held_qty}` per watchlist/position ticker from fresh account data. Tickers where nothing is possible (`{buy:0, sell:0}`) are dropped from today's active set — zero agent turns spent on them. The surviving set is recorded in `offered` (`specs/improvement.md` §4) so the night can count what each desk was asked to look at |
...
| Nightly 1 | Resolve + Reflect | Decisions at horizon (default **5 trading days**) or with invalidation hit are resolved: realized return + alpha vs SPY → `resolutions`; the `reflect` seat writes one reflection per decision → `resolutions.reflection` column only. Projection into journals is #57 (`specs/improvement.md` §8 (b)); the original Slack threads stay deferred |
| Nightly 2 | Score + Apply (no LLM) | `calibration/` recomputes per-seat skill and PM weights → `weights` table; `strategy.md` §6 ramp/probation/kill rules run on incubating sleeves; a desk with no measured edge is **narrowed** — its weight floor released, never retired (`specs/improvement.md` §2.1, §2.3). The PM's 11:00 brief reads the `weights` row as data |
| Weekly | Distill | the `distill` seat regenerates each seat's lessons file; the morning brief injects lessons, not the raw journal (`specs/improvement.md` §2.4) |
| Monthly | Propose | the `proposer` seat submits ≤1 pre-registered proposal per seat per open window; code projects it as a PR — every target, including a desk; the human merges or refuses; at horizon, code grades the prediction (`specs/improvement.md` §3) |
...
- **Journals:** per-agent append-only markdown via `state/journal.py` only — every call with its later resolution (realized return, alpha vs SPY, reflection). Injected into prompts as "recent record + lessons" — two brief sections from two sources: `journal` is the journal's recent entries (`recent_entries`), and `lessons` is the weekly-distilled `journals/<seat>.lessons.md` (`specs/improvement.md` §2.4), a derived file regenerated whole so that injected context stays bounded while the journal itself never is. Greppable, auditable, postable. Retrieval v1 is same-ticker + recency; a later optional upgrade is situation-similarity retrieval over reflections (TradingAgents' embedding-memory pattern) — do not build it before Phase 4.
- **Scoreboard:** a nightly table, not a weekly post — `weights` (`specs/improvement.md` §4): per-seat skill, calibration, behavioural rates, and the deterministic PM weight. (The Critic's **objection hit-rate** — objections on decisions that later resolved badly vs cleanly — is a Phase 3 column, added with the trade-pipeline critic.) The PM's brief reads it; the Proposer reads it; Ops projects it weekly. The feedback loop for tuning charters is the Proposer's PR, not a human reading the post.
...
2b. **Close the loop (`specs/improvement.md` §8).** In order of attribution cost: the scoring job and `weights` in the PM brief; reflections into the journal (#57); weekly lessons distillation; desk narrowing; the `proposals` table, the Proposer with one-number targets, and the PR projector; the evaluator and the Proposer's own scoreboard row. Charter and desk targets are gated on the taxonomy trigger (`improvement.md` §3.5), not scheduled. Before phase 3 because every seat added before the loop closes is another opinion tuned by hand; every seat added after it is graded from its first call.
...
4. **Running it.** Chaos tests, week-long sim, 30-day paper burn-in; merge or refuse the firm's proposals and count how many changes you had to write yourself (`VISION.md`); read 100+ traces by hand and write the taxonomy that unblocks charter proposals; add/retire seats. No new infrastructure.
