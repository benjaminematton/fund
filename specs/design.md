...
- **Scoreboard:** a nightly table, not a weekly post — `weights` (`specs/improvement.md` §4): per-seat skill, calibration, and the deterministic PM weight, plus the Critic's **objection hit-rate** (objections on decisions that later resolved badly vs cleanly). The PM's brief reads it; the Proposer reads it; Ops projects it weekly. The feedback loop for tuning charters is the Proposer's PR, not a human reading the post.
