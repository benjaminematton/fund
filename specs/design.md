...
| 11:30 | Execution | Trader places approved orders (`client_order_id` = ticket id; **`oto` order with a broker-side stop leg when the ticket carries `stop_price`**), posts fills to `#trade-log` linked to the decision thread |
